"""The evaluator permission matrix — the security boundary of the evaluation surface.

THIS MODULE IS A VERIFIER, NOT A DRIVER. A failing cell is a bug in the handler that owns the
route, never a wrong assertion here. Any relaxation of a 403 cell is an isolation violation and
...
# Non-evaluator exercise roles: 403 on every evaluation route.
...
async def _assert_denied_everywhere(c, f, h):
    """Every evaluation route 403s for this caller."""
    b = _base(f)
...
# --- explicit cross-evaluator rejections (must never be relaxed) -------------
...
async def test_evaluator_c_unassigned_to_this_report_gets_403_on_every_detail_route(
    ...
    async with client(migrated_db) as c:
        f = await _fixture(migrated_db, c, ah)
        b = _base(f)
        # …and the breakdown route gates too: 403, not 200 + [].
...
async def test_evaluator_whose_exercise_role_was_revoked_gets_403(migrated_db: async_sessionmaker) -> None:
    # The evaluation row survives (evaluator_id is RESTRICT); the caller does not.
    ah, _ = await ga_headers(migrated_db)
    async with client(migrated_db) as c:
        f = await _fixture(migrated_db, c, ah)
        ...
        # Reassignment of an orphaned evaluation is out of scope here.


# --- recall row of the matrix -------------------------------------------------
