"""The sole-writer rule as an executable contract: only ``rollup.py`` writes the grade columns.

The rule is what makes the grade auditable — one module to read, one place a number can come
from. It is easy to state in a docstring and easy to break with one well-meaning assignment in
a route, so it is checked here by reading the source tree.
...
def test_grade_version_is_only_ever_written_by_rollup() -> None:
    """The counter is incremented in exactly one function, so it cannot go backwards."""
    # Arrange / Act
