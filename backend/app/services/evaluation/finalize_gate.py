"""The finalize gate: when an exercise's evaluations feed the report grade.
...
async def resolve_finalize_policy(db: AsyncSession, exercise_id: uuid.UUID) -> str:
    """The exercise's finalize mode. An exercise with no ``scoring_config`` row gets the
    documented default — this never returns None.
    """
    mode = (
...
def is_gate_open(evaluations: Sequence[EvaluationFacts], mode: str) -> bool:
    """Whether ``under_evaluation`` -> ``evaluated`` is permitted right now.

    The empty counted set is CLOSED, not vacuously open: a report whose every evaluator was
    unassigned must not become ``evaluated`` with no grade behind it.

    Any mode other than ``any_can_finalize`` takes the strict branch, so a value that escaped
    the column's CHECK constraint fails closed.
    """
