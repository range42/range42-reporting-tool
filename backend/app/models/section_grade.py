...
class SectionGrade(Base, UUIDMixin, TimestampMixin):
    """Per-section grade + feedback from one evaluator.

    No row is ever created for a section whose source ``template_section_def.grade_mode`` is
    ``not_graded``: both the finalize condition and the rollup rule key off the row's absence.
    """
    ...
    # so a report delete must not be blocked by grade rows. A deliberate deviation from the
    # "…_section_id → RESTRICT" convention.
    ...
    # reads "rubric_scores IS NULL", which a JSON null does not satisfy.
